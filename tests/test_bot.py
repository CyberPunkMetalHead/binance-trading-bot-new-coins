...
class TestBot(TestCase):
    ...
    def test_convert_size(self):
        ...
        actual = self.FTX.broker.convert_size(config=self.FTX.config, ticker=ticker, price=40000)
        self.assertEqual(actual, 0.00075)

        actual = self.FTX.broker.convert_size(config=self.FTX.config, ticker=ticker, price=.008675309)
        self.assertEqual(actual, 3458.0900807106696)

        ticker = Ticker(ticker='BTCUSDT', base_ticker='BTC', quote_ticker="USDT")
        actual = self.Binance.broker.convert_size(self.Binance.config, ticker=ticker, price=48672.73020676)
    ...
    def test_update_below_sl(self):
        ...
        for key, value in self.FTX.orders.items():
            ...
            expected = Util.load_pickle(Config.TEST_DIR.joinpath('FTX_order_test_update_below_sl_expected'))
    ...
    def test_update_above_max(self):
        ...
        for key, value in self.FTX.orders.items():
            ...
            expected = Util.load_pickle(Config.TEST_DIR.joinpath('FTX_order_test_update_above_max_expected'))
    ...
    def test_update_above_tp(self):
        ...
        self.FTX.orders = Util.load_pickle(Config.TEST_DIR.joinpath('FTX_order_test_tsl_off'))

        for key, value in self.FTX.orders.items():
            self.FTX.update(key, value, current_price=60000)

            expected = Util.load_pickle(Config.TEST_DIR.joinpath('FTX_order_test_update_above_tp_expected'))
            expected['BTC/USDT'].sold_datetime = self.FTX.sold['BTC/USDT'].sold_datetime
            self.assertDictEqual(expected, self.FTX.sold)
    ...
    def test_update_below_tsl(self):
        ...
        for key, value in self.FTX.orders.items():
            ...
            expected = Util.load_pickle(Config.TEST_DIR.joinpath('FTX_order_test_update_below_tsl_expected'))
            expected['BTC/USDT'].sold_datetime = self.FTX.sold['BTC/USDT'].sold_datetime
            self.assertDictEqual(expected, self.FTX.sold)
